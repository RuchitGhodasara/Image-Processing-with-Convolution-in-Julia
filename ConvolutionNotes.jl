...
# ╔═╡ 4f8dd367-cbf6-4107-9c40-994e2f507135
...
### *Using Convolutions with `various Kernels`*
"""

# ╔═╡ 5d032454-cbed-4c09-a3b9-e2061d95f40d


# ╔═╡ 6cccf850-475f-4fa9-8855-f566f845e351
md"##

Inserting an Image from a device"

# ╔═╡ 6c4b1b2d-6fb4-4729-b493-b5d036e1dc07


# ╔═╡ d93ebb11-c343-430d-99cf-d1634eddeecb
md"""
## *Make your Choice*

_Chose Kernel to make image Blure or Sharp!_
"""


# ╔═╡ 129ba632-8bf6-4451-be78-4c301ca8f732
@bind choice Radio(["Blur","Sharp","Edge Dittection !","brightness"])
#varinfo(PlutoUI)

# ╔═╡ 20d1fed6-e335-4211-8532-34776d9aea09
md"##### Kernal"

# ╔═╡ eaf6f789-0b5f-42ca-96ed-9e57f1df9e76
md"##

### *Original Image*"

# ╔═╡ 89f79f5a-f9ad-47d1-a443-9e2d597553a5
md"### *Output Image*"

# ╔═╡ 751be6c0-0dc1-46b2-b6d7-6664b090b93a
if choice=="Edge Dittection !"
	md"""
	Brightness
	
	$(@bind amp html"<input type='range'>")
	
	*Scalling Factor 0 to 10*
	"""
end

# ╔═╡ fc01a3be-40cc-4f28-af9f-d3c4f86cea50
if choice=="brightness"
	md"""
	Brightness
	
	$(@bind intensity html"<input type='range'>")
	
	*Scalling Factor 0 to 5 times*
	"""
end

# ╔═╡ 51072f70-65b6-421a-84a2-ef08bbfbbb06


# ╔═╡ 7652cab2-d482-4156-b819-c3d6d137f622
md"##
...
only one line! :
 ...
 ###### _Output = convolve(image, kernel)_
"


# ╔═╡ 407b215a-64e4-4589-95bc-c0354de26807


# ╔═╡ 2e50b11e-b105-47bc-b23d-2c4cc6944b70
md"## Edge Detection

*Both X & Y Kernel have different effects, to Mearge both...*

1. Output = (**(convolve(image, kernel_x))** + **convolve(image, kernel_y)**)

*To Brighten the Output Multiply with some positive Integer {let say 3}...*

2. Output = 3*(**(convolve(image, kernel_x))** + **convolve(image, kernel_y)**)

*Get considered negetive ans. of convolution (inverted chages are also significant) too, take abs() => Detailed Image*

3. Output = 3*((abs.(**convolve(image, kernel_x)**) + abs.(**convolve(image, kernel_y)**)))

*Make Image Gray Scaled (As most of the color info has already lost)*

4. Output = 3*(Gray.(abs.(**convolve(image, kernel_x)**) + abs.(**convolve(image, kernel_y)**)))

**And 4th is final function used to compute our Output Image**
"

# ╔═╡ 42c2d258-3d7c-4ec2-b2b5-191ce489c538


# ╔═╡ 1be4323a-2583-4266-a69b-f3b10331a669
...
# ╔═╡ 5d9e1854-e493-4a48-8932-e5d39a0b5c86
function sharp_image_kernel()
	k=centered([-0.5 -1.0 -0.5
				-1.0 7.0 -1.0
			    -0.5 -1.0 -0.5])
	return k
end
...
if choice=="Blur"
	...
	md"*-->  Blurring the Image*"
	
elseif choice=="Edge Dittection !"
	kernel_x,kernel_y= Kernel.sobel()
	md"*-->  Dittecting the Edges*"
	
elseif choice=="Sharp"
	kernel=sharp_image_kernel()
	md"*-->  Sharping the Image*"
	
elseif choice=="brightness"
	md"*-->  Brighting the Image*"

else
	md"#### *Original Image only!*
	
	Select filter from the above list."
end
...
# ╔═╡ 2415d61f-b4ae-4ae6-a59d-f80cd7422483
if choice =="Sharp"
...
elseif choice == "Edge Dittection !"
	md"We use 2 Kernels for X and Y direction (As this is **Asymmetric**)"
...
# ╔═╡ 147c02f1-9fd8-4e61-9c57-a974b5879adf
if choice=="Edge Dittection !"
	md"$(show_colored_kernel(kernel_x))
	Kernal X"
end

# ╔═╡ f5be4e78-6c6b-42d2-9bee-ac38958f30f3
if choice=="Edge Dittection !"
	md"$(show_colored_kernel(kernel_y))
	Kernal Y"
end
...
# ╔═╡ 7dd5ff7e-faa2-439b-bfeb-734092ad4777
if choice=="Edge Dittection !"
	(amp/10)*(Gray.(abs.(convolve(image, kernel_x))+ abs.(convolve(image, kernel_y))))
elseif choice == "Sharp" || choice == "Blur"
    convolve(image, kernel)
elseif choice =="brightness"
	(intensity/50)*image
else
	md"Original image only, as no filters sellected !!
	$(image)"
end
...
# ╔═╡ 9f24e657-d78c-46be-ab40-00178b6c5740


# ╔═╡ 180612c0-5cc8-4a58-91a4-400a005b8cb6
